...
function MQTTServer(adapter, states) {
    ...
    function sendMessage2Client(client, topic, payload, qos, retain, cb) {
        const message = addMessageAttributes({topic: topic, payload: payload}, {qos: client._subsID[topic] ? client._subsID[topic].qos : undefined, retain: retain});
        if (message) {
    ...
    function startServer(config, socket, server, port, bind, ssl, ws) {
        ...
        (server || socket).listen(port, bind, () =>
            adapter.log.info(`Starting MQTT${ws ? '-WebSocket' : ''}${ssl ? ' (Secure)' : ''}${config.user ? ' authenticated' : ''} server on ${bind || '0.0.0.0'}:${port}`));
    }
